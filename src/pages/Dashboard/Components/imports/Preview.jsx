...
const uploadPhoto = async (file, clientId) => {
  ...
  const filePath = `${clientId}/photo/${crypto.randomUUID()}.${ext}`; // ← هنا استخدمنا clientId بدل الاسم
...
const uploadDesign = async (file, clientId, isPreview = false) => {
  ...
  const filePath = `${clientId}/${folder}/${crypto.randomUUID()}.${ext}`; // ← نفس الشيء
      ...
      >
        <input
          ...
          onChange={(e) => {
            ...
            if (file) onChange(file);  // فقط File هنا
            e.target.value = '';
          }}
        />        {imageSrc ? (
...
export const ClientCard = ({
  ...
  const [previewDesigns, setPreviewDesigns] = useState([null, null, null, null]); // ← هاي السطر الفارق
  ...
  const handleSubmit = async () => {
    ...
    try {
      const newClient = await createClient({
        ...
        is_verified: verify,
        is_visible: true,
        photo: '' // مؤقت، بنحدثه بعد رفع الصورة
      });

      // استخدم client.id كاسم الفولدر
      ...
      // جمع كل التصاميم (الـ 4 preview + show more)
      const allDesigns = [
